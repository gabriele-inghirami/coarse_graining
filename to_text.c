...
void interpret_output_content(int num)
{
...
fread(&output_content,sizeof(int),1,fin);
...
if (total_baryon_included > 0) offset+=1;
if (resonances_included > 0) offset_res+=3*nr;
...
if (resonances_included > 0) fprintf(fout,"Number of resonances: %12d  \n",nr);
	...
	for(j=0;j<ny;j++)
	{
		for(k=0;k<nz;k++)
            ...
            if (resonances_included > 0) {
			    for(r=0;r<nr;r++)
			    {
				    fread(&datap[h+3*r],sizeof(double),1,fin);
				    fread(&datap[h+3*r+1],sizeof(double),1,fin);
				    fread(&datap[h+3*r+2],sizeof(double),1,fin);
			    }
                h+=3*r;
            }
		}
	...
	for(j=0;j<ny;j++)
	{
		for(k=0;k<nz;k++)
        {
            ...
            if (resonances_included > 0)
            {
			    for(r=0;r<nr;r++)
			    {
                    fprintf(fout, "Resonance kind: %4d, total cell number: %14ld, number density: %14.9e, energy density: %14.9e\n",r, (long int) datap[3*r+h], datap[3*r+h+1], datap[3*r+h+2]);
			}
            fprintf(fout,"\n");
            h+=3*r;
            }
