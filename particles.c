...
const char* associate_particle_array_index(int index)
{
switch(index) {
case(0):
...
#elif defined(SMASH)
void get_had_prop(char* id_string, int id_string_len, int* s, int* B, int* Jtot)
{
	...
	if(end>2)
	   ...
	   { 
	      ...
	      *B=1; //3 quarks: it is a baryon
	      if(*Jtot==9) *Jtot=10; //this is an expection for N(2200), N(2250) and Lambda(2450) with J=9/2, so that 2J+1=10, but in their ID the last number is 9
	      if((*Jtot%2)!=0)
	      { //it must be a fermion, 2J+1 must be even and, if divided by 2 there is a reminder, it is odd...
		  printf("Sorry, but there is something wrong. In particles.txt I have found a baryon with integer spin... I quit.\n");
	          exit(6);
	      }
	   }
	   else
	   { 
	      ...
	      *B=0; //2 quarks: it is a meson
		  ...
		  printf("Sorry, but there is something wrong. In particles.txt I found a meson with half-integer spin... I quit.\n");
	          exit(6);
	      }
	   }
	}
	...
	*s=strangeness[q1-1]+strangeness[q2-1];
	if(q3>0) *s=*s+strangeness[q3-1];
	else *s=-*s; //if q3==0 is a meson and in that case the s are actually anti-s
	if((q3==0) && (q1==q2)) *s=0; //it is a s-antis state

}

int fill(pinfo* plist)
{
	int n=0;
	int i;
	wchar_t buf[200];
	...
	if(finp==NULL) {
		printf("Sorry, but it is not possible to open particles.txt, containing the data of the hadron species managed by SMASH... I quit.\n\n");
		exit(5);
	}
	...
	while(fgetws(buf,200,finp)!=NULL)
	{
		...
		{
			swscanf(buf,L"%ls %lf %e %c %s %s %s %s",&plist[n].name, &plist[n].mass, &float_bb, &par, &pdg_id_stringA, &pdg_id_stringB, &pdg_id_stringC, &pdg_id_stringD);
			plist[n].pdg_id=atoi(pdg_id_stringA);
			get_had_prop(pdg_id_stringA, strlen(pdg_id_stringA), &plist[n].strangeness, &plist[n].baryon_num, &plist[n].spin);
			//printf("%-3d   %-11ls  %12.4lf %10d   B: %1d   s: %2d   2J+1: %2d\n",n, plist[n].name, plist[n].mass, plist[n].pdg_id, plist[n].baryon_num, plist[n].strangeness, plist[n].spin);
                        n=n+1;

			if((pdg_id_stringB[0]=='#') || (pdg_id_stringB[0]==' ') || (pdg_id_stringB[0]=='\0') || (pdg_id_stringB[0]=='\n')) continue;
			...
			if(tmp_pdg_idB!=0)
			{
				...
				plist[n].pdg_id=tmp_pdg_idB;
			        get_had_prop(pdg_id_stringB, strlen(pdg_id_stringB), &plist[n].strangeness, &plist[n].baryon_num, &plist[n].spin);
			        //printf("%-3d   %-11ls  %12.4lf %10d   B: %1d   s: %2d   2J+1: %2d\n",n, plist[n].name, plist[n].mass, plist[n].pdg_id, plist[n].baryon_num, plist[n].strangeness, plist[n].spin);
                                n=n+1;
			        strncpy(pdg_id_stringB,zero_string,sizeof(pdg_id_stringB));
			}

			if((pdg_id_stringC[0]=='#') || (pdg_id_stringC[0]==' ') || (pdg_id_stringC[0]=='\0') || (pdg_id_stringC[0]=='\n')) continue;
			...
			if(tmp_pdg_idC!=0)
			{
				...
				plist[n].pdg_id=tmp_pdg_idC;
			        get_had_prop(pdg_id_stringC, strlen(pdg_id_stringC), &plist[n].strangeness, &plist[n].baryon_num, &plist[n].spin);
			        //printf("%-3d   %-11ls  %12.4lf %10d   B: %1d   s: %2d   2J+1: %2d\n",n, plist[n].name, plist[n].mass, plist[n].pdg_id, plist[n].baryon_num, plist[n].strangeness, plist[n].spin);
                                n=n+1;
			        strncpy(pdg_id_stringC,zero_string,sizeof(pdg_id_stringC));
			}

			if((pdg_id_stringD[0]=='#') || (pdg_id_stringD[0]==' ') || (pdg_id_stringD[0]=='\0') || (pdg_id_stringD[0]=='\n')) continue;
			...
			if(tmp_pdg_idD!=0)
			{
				...
				plist[n].pdg_id=tmp_pdg_idD;
			        get_had_prop(pdg_id_stringD, strlen(pdg_id_stringD), &plist[n].strangeness, &plist[n].baryon_num, &plist[n].spin);
			        //printf("%-3d   %-11ls  %12.4lf %10d   B: %1d   s: %2d   2J+1: %2d\n",n, plist[n].name, plist[n].mass, plist[n].pdg_id, plist[n].baryon_num, plist[n].strangeness, plist[n].spin);
                                n=n+1;
