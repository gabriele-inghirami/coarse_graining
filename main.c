  ...
  if ((strncmp (argv[1], "comp", 4) == 0) || (strncmp (argv[1], "avg", 3) == 0))
    {
      ...
      big_arrays_allocated_mem
          = ((size_t)(14 * sizeof (double) * nt * np) + (size_t)(4 * sizeof (double) * nt)) * ((size_t)(nx * ny * nz));
      if (big_arrays_allocated_mem > max_memory_allocatable_data)
        {
          ...
          printf ("The ram memory usable by the program according to the parameter MAX_MEMORY_ALLOC in "
                  "particle_definitions.h (which sets the fraction of the available memory) is %zu\n.",
  ...
  if ((strncmp (argv[1], "comp", 4) == 0))
  ...
  else if (strncmp (argv[1], "avg", 3) == 0)
    {
      if (big_arrays_allocated_mem * 2 > max_memory_allocatable_data)
        {
          printf ("The memory required for the allocation of the main arrays (in the average procedure we need them "
                  "twice) is %zu.\n",
