...
void write_results(char *outputprefix, double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, double *Jr, double *Tr, long int *Pnum, long int *Rnum, long int nevents)
{
	int h,i,j,k,p,r;
	...
	if(first_time==0)//we print the grid informations
	{
		...
		*info_filename='\0';
		...
		for(i=0;i<nx;i++) fprintf(finfo," %4d       %9.4lf\n",i,xmin+(i+0.5)*dx);
		fprintf(finfo,"\n");
		fprintf(finfo,"Index:   Y coordinate:\n");
		for(j=0;j<ny;j++) fprintf(finfo," %4d       %9.4lf\n",j,ymin+(j+0.5)*dy);
		fprintf(finfo,"\n");
		fprintf(finfo,"Index:   Z coordinate:\n");
		for(k=0;k<nz;k++) fprintf(finfo," %4d       %9.4lf\n",k,zmin+(k+0.5)*dz);
		fprintf(finfo,"\n");
...
void write_densities(char *outputprefix, double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, double *Jr, double *Tr, long int *Pnum, long int *Rnum, long int nevents)
{
	int h,i,j,k,p,l,r;
	char *output_filename_Tp;
	char *info_filename;
    ...
    if(empty_array==NULL)
    {
        printf("Unable to create the empty_array in function write_densities in io.c. This is a weird situation and I prefer to quit.\n");
        exit(4);
    }
	...
	for(h=0;h<nt;h++)
	{
	  if(h==0) printf("Now computing and printing densities of the total %ld events\n",nevents);
	  snprintf(time_suffix,8,"%07.3lf",time_int_array[h]);
	  strncpy(&time_suffix[3],"_",1);//we replace the dot with an underscore
	  ...
	  for(i=0;i<nx;i++)
	  {
			for(j=0;j<ny;j++)
			{
				for(k=0;k<nz;k++)					
				{
					...
					jf_arg=(Jb[J0+JBL]*Jb[J0+JBL]-Jb[J1+JBL]*Jb[J1+JBL]-Jb[J2+JBL]*Jb[J2+JBL]-Jb[J3+JBL]*Jb[J3+JBL]);
					if(jf_arg>0)
					{	jf=sqrt(jf_arg);					
						u4[0]=Jb[J0+JBL]/jf;
						u4[1]=Jb[J1+JBL]/jf;
						u4[2]=Jb[J2+JBL]/jf;
						u4[3]=Jb[J3+JBL]/jf;
                        //printf("*** Point with coordinates: %d  %d  %d  %e  %e  %e\n",i,j,k,xmin+(i+0.5)*dx,ymin+(j+0.5)*dy,zmin+(k+0.5)*dz);
                        //printf("*** Point with coordinates: %e  %e  %e\n",xmin+(i+0.5)*dx,ymin+(j+0.5)*dy,zmin+(k+0.5)*dz);
                        //printf("u components: %e  %e  %e  %e\n",u4[0], u4[1], u4[2], u4[3]);
						rho=(Jb[J0+JBL]*u4[0]-Jb[J1+JBL]*u4[1]-Jb[J2+JBL]*u4[2]-Jb[J3+JBL]*u4[3])/(cell_volume*nevents);
                        //printf("Jb components and rho: %e  %e  %e  %e  %e\n",Jb[J0+JBL], Jb[J1+JBL], Jb[J2+JBL], Jb[J3+JBL], rho);
						fwrite(&rho,sizeof(double),1,fTp);
                        for(l=1;l<4;l++) vel_B[l-1]=u4[l]/u4[0];
                        fwrite(vel_B,sizeof(double),3,fTp);                           
                        //printf("vel components: %lf  %lf  %lf  \n",vel_B[0],vel_B[1],vel_B[2]);
                        #ifdef INCLUDE_TOTAL_BARYON
						rho_t=(Jt[J0+JBL]*u4[0]-Jt[J1+JBL]*u4[1]-Jt[J2+JBL]*u4[2]-Jt[J3+JBL]*u4[3])/(cell_volume*nevents);
						fwrite(&rho_t,sizeof(double),1,fTp);
                        #endif
						rho_c=(Jc[J0+JBL]*u4[0]-Jc[J1+JBL]*u4[1]-Jc[J2+JBL]*u4[2]-Jc[J3+JBL]*u4[3])/(cell_volume*nevents);
						rho_s=(Js[J0+JBL]*u4[0]-Js[J1+JBL]*u4[1]-Js[J2+JBL]*u4[2]-Js[J3+JBL]*u4[3])/(cell_volume*nevents);
                        fwrite(&rho_c,sizeof(double),1,fTp);
                        fwrite(&rho_s,sizeof(double),1,fTp);
                        //printf("Jt components and rho_t: %e  %e  %e  %e  %e\n",Jt[J0+JBL], Jt[J1+JBL], Jt[J2+JBL], Jt[J3+JBL], rho_t);
                        //printf("Jc components and rho_c: %e  %e  %e  %e  %e\n",Jc[J0+JBL], Jc[J1+JBL], Jc[J2+JBL], Jc[J3+JBL], rho_c);
                        //printf("Js components and rho_s: %e  %e  %e  %e  %e\n",Js[J0+JBL], Js[J1+JBL], Js[J2+JBL], Js[J3+JBL], rho_s);
                        ...
                        for(l=0;l<4;l++) Ic_diffusion[l]=Jc[l+JBL]/(cell_volume*nevents)-rho_c*u4[l];
                        for(l=0;l<4;l++) Is_diffusion[l]=Js[l+JBL]/(cell_volume*nevents)-rho_s*u4[l];
                        for(l=0;l<4;l++) {
                            if(fabs(Ic_diffusion[l]-Ic_diffcheck[l]/(cell_volume*nevents))>1.e-10) printf("Warning, mismatching in charge diffusion currents at i=%d, j=%d, k=%d! %lf vs %lf\n",i,j,k, Ic_diffusion[l], Ic_diffcheck[l]);
                            if(fabs(Is_diffusion[l]-Is_diffcheck[l]/(cell_volume*nevents))>1.e-10) printf("Warning, mismatching in strange diffusion currents at i=%d, j=%d, k=%d! %lf vs %lf\n",i,j,k, Is_diffusion[l], Is_diffcheck[l]);
                                         }
                        fwrite(Ic_diffusion,sizeof(double),4,fTp);
                        fwrite(Is_diffusion,sizeof(double),4,fTp);
                        //printf("Ic diffusion components: %e  %e  %e  %e\n",Ic_diffusion[0],Ic_diffusion[1],Ic_diffusion[2], Ic_diffusion[3]);
                        //printf("Is diffusion components: %e  %e  %e  %e\n",Is_diffusion[0],Is_diffusion[1],Is_diffusion[2], Is_diffusion[3]);
                        //printf("\n\n");
						for(p=0;p<np;p++)					
						{
						...
						for(r=0;r<nr;r++)					
						{
							rho=(Jr[J0+RNLOC]*u4[0]-Jr[J1+RNLOC]*u4[1]-Jr[J2+RNLOC]*u4[2]-Jr[J3+RNLOC]*u4[3])/(cell_volume*nevents);
							eps=((u4[0]*Tr[T00+RTLOC]-u4[1]*Tr[T10+RTLOC]-u4[2]*Tr[T20+RTLOC]-u4[3]*Tr[T30+RTLOC])*u4[0]-
								(u4[0]*Tr[T01+RTLOC]-u4[1]*Tr[T11+RTLOC]-u4[2]*Tr[T21+RTLOC]-u4[3]*Tr[T31+RTLOC])*u4[1]-
								(u4[0]*Tr[T02+RTLOC]-u4[1]*Tr[T12+RTLOC]-u4[2]*Tr[T22+RTLOC]-u4[3]*Tr[T32+RTLOC])*u4[2]-
								(u4[0]*Tr[T03+RTLOC]-u4[1]*Tr[T13+RTLOC]-u4[2]*Tr[T23+RTLOC]-u4[3]*Tr[T33+RTLOC])*u4[3])/(cell_volume*nevents);
					...
					else
					{
						...
						fwrite(empty_array,sizeof(double),offset,fTp);//it includes rho,velB,rho_t,rho_c, rho_s, Ic_diffusion,Is_diffusion, rho and eps for all hadrons
					 }						
				}
			}				
	   }
...
void read_data(char *inputfile, double *data_Tp, double *data_Jp, double *data_Jb,  double *data_Jc,  double *data_Js, double *data_Jt, double *data_Jr, double *data_Tr, long int *data_Pnum, long int *data_Rnum, long int *nevents, double *data_time)
{
	  FILE *infile;
      int np2, nx2, ny2, nz2;
      ...
      fread(&output_content_info2,sizeof(int),1,infile);
      ...
      if((nx != nx2) || (ny != ny2) || (nz != nz2) || (dx != dx2) || (dy != dy2) || (dz != dz2) || (xmin != xmin2) || (ymin != ymin2) || (zmin != zmin2)) {
        printf("Error: mismatching between the dimension and/or resolution of the old and the new grid!!!\n");
        printf("The old values are: nx=%d, ny=%d, nz=%d, dx=%6.4lf, dy=%6.4lf, dz=%6.4lf, xmin=%6.4lf, ymin=%6.4lf, zmin=%6.4lf\n", nx, ny, nz, dx, dy, dz, xmin, ymin, zmin); 
        printf("The new values in %s are: nx2=%d, ny2=%d, nz2=%d, dx2=%6.4lf, dy2=%6.4lf, dz2=%6.4lf, xmin2=%6.4lf, ymin2=%6.4lf, zmin2=%6.4lf\n", inputfile, nx2, ny2, nz2, dx2, dy2, dz2, xmin2, ymin2, zmin2);
        printf("Sorry, but I have to quit..\n");
        fclose(infile);
        exit(6); 
      }
      ...
      #else
	  fread(data_Jr,sizeof(double),1,infile);
	  fread(data_Tr,sizeof(double),1,infile);
      #endif
	  fread(data_Pnum,sizeof(long int),nx*ny*nz*np,infile);
      #ifdef INCLUDE_RESONANCES
	  fread(data_Rnum,sizeof(long int),nx*ny*nz*nr,infile);
      #else
	  fread(data_Rnum,sizeof(long int),1,infile);
      #endif
	  printf("%s read.\n",inputfile);
	  fclose(infile);
}
