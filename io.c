...
void write_densities(char *outputprefix, double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, long int *Pnum, long int nevents)
{
	int h,i,j,k,p,l;
	char *output_filename_Tp;
	char *info_filename;
			...
			for(j=0;j<ny;j++)
			{
				for(k=0;k<nz;k++)					
				{
					...
					else
					{
						...
						fwrite(empty_array,sizeof(double),15+np*3,fTp);//it includes rho,velB,rho_t,rho_c, rho_s, Ic_diffusion,Is_diffusion, rho and eps for all hadrons
					 }						
				}
			}				
	   }
...
void read_data(char *inputfile, double *data_Tp, double *data_Jp, double *data_Jb,  double *data_Jc,  double *data_Js, double *data_Jt, long int *data_Pnum, long int *nevents, double *data_time)
{
	  FILE *infile;
          int np2, nx2, ny2, nz2;
          double dx2, dy2, dz2;
