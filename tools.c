...
void
check_input_files (char **infiles, int narg)
{
  int i;
  FILE *fp;
...
void
get_timesteps (char *timefile, int *ntimesteps, double **timesteps_array)
{
  ...
  if (ftint == NULL)
    ...
    {
      *ntimesteps = 0;
      while (fscanf (ftint, "%lf", &time_intervals_temporary_array[*ntimesteps]) != EOF)
        (*ntimesteps)++;
    }
  fclose (ftint);
