          ...
          for (k = 0; k < nz; k++)
            {
              ...
              if (success == 1)
                {
                  ...
                  rho_b = (Jb[J0 + JBL] * u4[0] - Jb[J1 + JBL] * u4[1] - Jb[J2 + JBL] * u4[2] - Jb[J3 + JBL] * u4[3])
                          / NF;
                  rho_c = (Jc[J0 + JBL] * u4[0] - Jc[J1 + JBL] * u4[1] - Jc[J2 + JBL] * u4[2] - Jc[J3 + JBL] * u4[3])
                          / NF;
                  rho_s = (Js[J0 + JBL] * u4[0] - Js[J1 + JBL] * u4[1] - Js[J2 + JBL] * u4[2] - Js[J3 + JBL] * u4[3])
                          / NF;
                  fwrite (&rho_b, sizeof (double), 1, fde);
                  fwrite (&rho_c, sizeof (double), 1, fde);
                  fwrite (&rho_s, sizeof (double), 1, fde);
                  Ib_diffcheck[0] = (1 - u4[0] * u4[0]) * Jb[J0 + JBL] + u4[0] * u4[1] * Jb[J1 + JBL]
                                    + u4[0] * u4[2] * Jb[J2 + JBL] + u4[0] * u4[3] * Jb[J3 + JBL];
                  Ib_diffcheck[1] = -u4[0] * u4[1] * Jb[J0 + JBL] + (1 + u4[1] * u4[1]) * Jb[J1 + JBL]
                                    + u4[1] * u4[2] * Jb[J2 + JBL] + u4[1] * u4[3] * Jb[J3 + JBL];
                  Ib_diffcheck[2] = -u4[0] * u4[2] * Jb[J0 + JBL] + u4[2] * u4[1] * Jb[J1 + JBL]
                                    + (1 + u4[2] * u4[2]) * Jb[J2 + JBL] + u4[2] * u4[3] * Jb[J3 + JBL];
                  Ib_diffcheck[3] = -u4[0] * u4[3] * Jb[J0 + JBL] + u4[3] * u4[1] * Jb[J1 + JBL]
                                    + u4[3] * u4[2] * Jb[J2 + JBL] + (1 + u4[3] * u4[3]) * Jb[J3 + JBL];
                  ...
                  for (l = 0; l < 4; l++)
                    Ib_diffusion[l] = Jb[l + JBL] / NF - rho_b * u4[l];
                  for (l = 0; l < 4; l++)
                    Ic_diffusion[l] = Jc[l + JBL] / NF - rho_c * u4[l];
                  for (l = 0; l < 4; l++)
                    Is_diffusion[l] = Js[l + JBL] / NF - rho_s * u4[l];
                  for (l = 0; l < 4; l++)
                    {
                  ...
                  for (p = 0; p < np; p++)
                    {
                      rho = (Jp[J0 + JPL] * u4[0] - Jp[J1 + JPL] * u4[1] - Jp[J2 + JPL] * u4[2] - Jp[J3 + JPL] * u4[3])
                            / (cell_volume * nevents);
              ...
              if ((argc == 4) && (success == 1))
                { // we compute also Tmunu in the Landau frame
                  glf1 = u4[0] + 1;
                  ...
                  // we save the informations about the number of particles and the grid size because, for a few bytes
                  // more of disk space, we can perform a consistency check when averaging
