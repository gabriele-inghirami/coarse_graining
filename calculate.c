...
int continue_flag=0;
//typedef struct smash_record{double t; double  x; double  y; double z; double mass; double p0; double px; double py; double pz; int32_t pdg; int32_t ID; int32_t charge;} smash_record;
...
fpos_t begin_of_event_file_position;//the position of the beginning of the current event in the opened SMASH binary output file
...
for(idx=0;idx<nf;idx++)
{
    ...
    printf("Working on file %s\n", files[idx+start_index]);
    fread(bb_char,sizeof(char),4,infile);
    fread(&format_version,sizeof(uint16_t),1,infile);
    /*
    if((int)format_version!=4)
    {
	    printf("Error! This program is designed to read SHASH binary output version 4, but you provided a file written according to version %u\n.",format_version);
	    exit(7);
    }*/
    fread(&format_variant,sizeof(uint16_t),1,infile);
    /*if(format_variant!=0)
    {
	    printf("Error! This program is designed to read SHASH format variant 0 (normal), but you provided a file written according to variant %u\n.",format_variant);
	    exit(7);
    }*/
    fread(&smash_vprint_len,sizeof(uint32_t),1,infile);
    fread(smash_ver,sizeof(char),smash_vprint_len,infile);
    smash_ver[smash_vprint_len]='\0';
    printf("Reading %s binary output, OSCAR format version %u, variant %u\n",smash_ver,format_version,format_variant);
    fgetpos(infile,&begin_of_event_file_position);
    if(b_selection_sm==0) look_at_b=0;
    else look_at_b=1;
    event_number_check=0;
    while(1)
    {
        if(fread(&p_char_at_smash_block_start,sizeof(char),1,infile)<1) break;
        if(!((p_char_at_smash_block_start=='p') || (p_char_at_smash_block_start=='f')))
        {
	    printf("Error in reading file %s. I did not find the p character at the beginning of a block header, but %c. I quit.\n", files[idx+start_index],p_char_at_smash_block_start);
	    exit(8);
        }
	if(p_char_at_smash_block_start=='p')
	{
           fread(&n_part_lines,sizeof(uint32_t),1,infile);
	   ...
	   else
	   {
	     //we read the first double, to check the time
             fread(&out_time,sizeof(double),1,infile);
	     ...
	     {
               //we read a block of particles at time, we check if the impact parameter is within the desired range and, if so, we analize the data
               //but first, we check if we have allocated enough room
		   ...
		   buffer_smash_tmp=NULL;
               ...
               fread(buffer_smash,smash_record_size,n_part_lines,infile);
	       ...
	       for(k=0;k<n_part_lines;k++)
	       {
	           ...
	           buffer_smash+=2*sizeof(uint32_t);//we move fwd 2 unsigned int, because we skip the particle internal ID info
	...
	{
	    fread(&event_number,sizeof(uint32_t),1,infile);
	    if(event_number!=event_number_check)
	    {
		    printf("There is a discrepancy between the number of events according to the file %s (%u) and what we counted so far (%u)... Please, check!\n", files[idx+start_index],event_number,event_number_check);
		    exit(8);
            }
	    else
	    {
	    ...
	    fread(&b_smash,sizeof(double),1,infile);
	    fread(&empty_char,sizeof(char),1,infile);
	    ...
	    {
		    event_number_check=event_number_check+1; //we discard the event, but we update the event_number_check variable
	            fgetpos(infile,&begin_of_event_file_position);
	    }
...
int avg(char **files, int ninfiles, double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, long int *Pnum, long int *nevents)
{
	...
	read_data(inputfile,Tp2,Jp2,Jb2,Jc2,Js2,Jt2,Pnum2,&nevents2,&time_check);
	  ...
	  {
		printf("Mismatching between the time in file %s (%lf) and corresponding entry %d in time array obtained from file %s.\n",input_filename_Tp,time_check,h,files[ninfiles-1]);
		printf("I quit.\n");
	        exit(5);
	  }
        }
       	...
       	for(i=0;i<nx;i++)
	{
	   for(j=0;j<ny;j++)
	   {
	      for(k=0;k<nz;k++)					
	      {
                 for(p=0;p<np;p++)					
	         {
	            for(r=0;r<10;r++) Tp[r+TLOC]+=Tp2[r+TLOC];
		 }
	      }
	    }				
	}
	      ...
	      for(k=0;k<nz;k++)					
           {
              for(p=0;p<np;p++)					
		 {
                  for(r=0;r<4;r++) Jp[r+JPL]+=Jp2[r+JPL];
		 }
	      }
	   }				
	}
...
int process_data(double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, long int *Pnum, long int nevents, pdata *pdata_input)
{
    ...
    {
	  printf("I just start computing the energy-momentum tensor, but I have no particle data. Something went wrong... I quit.\n");
	  exit(2);
	}
