          ...
          if (strncmp (buffer, "UQMD", 4) == 0)
            {
              ...
              if (((b >= bmin) && (b <= bmax)) || (b_selection == 0))
                ...
                take_event = -1;
              for (k = 0; k < 14; k++)
                gl_bb = getline (&buffer, &buffer_size, infile); // we move 14 lines forward
            }
  ...
  for (idx = 0; idx < nf; idx++)
    {
      ...
      while (1)
        {
          if (fread (&p_char_at_smash_block_start, sizeof (char), 1, infile) < 1)
            break;
...
int
avg (char **files, int ninfiles, double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt,
     long int *Pnum, long int *nevents)
{
...
int
process_data (double *Tp, double *Jp, double *Jb, double *Jc, double *Js, double *Jt, long int *Pnum, long int nevents,
              pdata *pdata_input)
{
  int h, i, j, k, p;
  int jsign;
