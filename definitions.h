...
/**
* \struct pdata (UrQMD) 
* It contains most of the information provided by UrQMD about a particle. However, please, note that the first member is the time index instead of the actual time. 
* The pdata components are:
*  - t_intex : time index
*  - x       : the x coordinate
*  - y       : the y coordinate
*  - z       : the z coordinate
...
typedef struct pdata{int t_index; double x; double y; double z; double en; double px; double py; double pz; double m; int itype; int iso3; int charge; struct pdata *next;} pdata;

#elif defined(SMASH)
/**
* \struct pdata (SMASH)
* It contains most of the information provided by SMASH about a particle. However, please, note that the first member is the time index instead of the actual time. 
* The pdata components are:
*  - t_intex : time index
*  - x       : the x coordinate
*  - y       : the y coordinate
...
*  - *next   : a pointer to the memory location of the next pdata structure
*/
typedef struct pdata{int t_index; double x; double y; double z; double en; double px; double py; double pz; double m; int pdg_id; int charge; struct pdata *next;} pdata;
#else
#error "You must define either SMASH or URQMD in definitions.h - compilation failed."
#endif
...
*/
typedef struct pinfo{int pdg_id; wchar_t name[16]; double mass; int spin; int strangeness; int baryon_num; } pinfo;
...
enum {T00=0,T01,T02,T03,T11,T12,T13,T22,T23,T33}; /**< \enum The flattened indexes of the energy momentum tensor components */
enum {J0=0,J1,J2,J3}; /**< \enum The flattened indexes of four momentum components */
 ...
 * Unkown particles go in the last Tmunu index.
*/
#ifdef URQMD
...
/** @brief it write the results into an output file. These output files contain all the necessary informations for further averaging.
*
*   @param[in] outputprefix the prefix of the output files
*
*   @param[in] *Tp a pointer to the energy momentum tensor of the particle species
