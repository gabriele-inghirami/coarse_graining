...
for(i=0;i<nx;i++)
{
	for(j=0;j<ny;j++)
	{
		for(k=0;k<nz;k++)
                {
                        if(((chosen_dir==0) && (i==chosen_index)) || ((chosen_dir==1) && (j==chosen_index)) || ((chosen_dir==2) && (k==chosen_index)))
                        {
      ...
      if (ret_it == 0)
      {
          ...
          exit(4);
			  ...
			  fprintf(fout,"%10.6e  ",edens); //total energy density
                          ...
                          if(((chosen_dir==0) && (inner_counter==nz)) || ((chosen_dir==1) && (inner_counter==nz)) || ((chosen_dir==2) && (inner_counter==ny)))
                          {
                             inner_counter=0;
                             fprintf(fout,"\n");
                          }
